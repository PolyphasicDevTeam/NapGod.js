...
async function get(args, message, dry) {
  let msg = "";
  arg = message.content.slice(config.prefix.length + getLogs.length, message.content.length).trim()
  console.log("CMD   : GET")
      ...
      if (res) {
        res.forEach(log => {
          ...
          if (!filter || !filter.userName) {
            msg += `Username: ${log.userName}\n`
          }
          msg += `Schedule: ${log.schedule}\n`
          if (log.attempt) {
            msg += `Attempt: ${log.attempt}\n`
          }
          msg += `Day: ${log.day}\n`
          if (log.daySegments) {
            msg += `Segments: ${log.daySegments}\n`
          }
          msg += `Moods: ${log.moods}\n`
          msg += `Awake difficulty: ${log.awakeDifficulty}\n`
          msg += `Sleep time: ${log.sleepTime}\n`
          if (log.oversleepTime) {
            msg += `Oversleep time: ${log.oversleepTime}\n`
          }
          if (log.napsNumber) {
            msg += `Naps number: ${log.napsNumber}\n`
          }
          if (log.logMessage) {
            msg += `LogMessage: ${log.logMessage}\n`
          }
          if (log.attachment) {
            msg += `Attachment: ${log.attachment}\n`
          }
          msg += "```";
        });
        if (!dry){

...
whitelist = [
  ...
  '459124864847052810', //Ktk
  '507042428767698955', //Fei
];
...
async function mset(args, message, dry) {
  ...
  arg = message.content
    .slice(config.prefix.length + 4, message.content.length)
    .trim();
  ...
  if (uid != '') {
    ...
    if (member != null) {
      ...
      set([args[0], args[1]], message, dry, member.user, member, true).then(
        (res) => {
          if (!res) {
            msg =
              'Valid options are `+mset [schedule-name] [napchart-link] [username]`';
